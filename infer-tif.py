...
# python3 infer-tif.py --model_path_prefix model/fidelity-413/inference --file CACONT-2023_00035646.TIF
...
def generate_images(file):
    ...
    name = tail.split('.')[0]
    OUTPUT_DIR = os.path.join(BASE_DIR, 'images', name)
    if not os.path.exists(OUTPUT_DIR):
        os.mkdir(OUTPUT_DIR)
    output_files = list()
    images = Image.open(os.path.join(INPUT_DIR, file))
    for i, image in enumerate(ImageSequence.Iterator(images)):
        image.save(os.path.join(OUTPUT_DIR, f'page{i}.jpg'))
        output_files.append(os.path.join(OUTPUT_DIR, f'page{i}.jpg'))
    return output_files
...
def parse_args():
    ...
    parser.add_argument("--file", type=str, required=True, help="TIF File")
...
def main():
    args = parse_args()
    images = generate_images(args.file)
