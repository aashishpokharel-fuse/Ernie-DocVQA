...
class InferBackend(object):
    ...
    def infer(self, input_dict: dict):
        ...
        # print("INPUT HANDLES", self.input_handles[0])
        self.predictor.run()
...
class Predictor(object):
    def __init__(self, args):
        ...
        self._PrelimPrediction = collections.namedtuple(
            "PrelimPrediction", ["feature_index", "start_index", "end_index", "start_logit", "end_logit", "confidence_score"]
        )
    ...
    def postprocess_mrc(self, preds, max_answer_length=64, n_best_size=5):
        ...
        # Calculate min and max logits for normalization
        # all_start_logits = np.concatenate([preds[0][idx] for idx in range(len(preds[0]))])
        # all_end_logits = np.concatenate([preds[1][idx] for idx in range(len(preds[1]))])
        # min_start_logit = np.min(all_start_logits)
        # max_start_logit = np.max(all_start_logits)
        # min_end_logit = np.min(all_end_logits)
        # max_end_logit = np.max(all_end_logits)
        ...
        for ei, example_id in enumerate(self.examples_cache["name"]):
            ...
            for idx in features_ids:
                ...
                for start_index in start_indexes:
                    for end_index in end_indexes:
                        ...
                        # normalized_start_logit = (start_logit - min_start_logit) / (max_start_logit - min_start_logit)
                        # normalized_end_logit = (end_logit - min_end_logit) / (max_end_logit - min_end_logit)
                        # confidence_score = (normalized_start_logit + normalized_end_logit) / 2
                        # print("START LOGIT", start_logits[start_index])
                        # print("END LOGIT", end_logits[end_index])
                        prelim_predictions.append(
                            self._PrelimPrediction(
                                ...
                                end_logit=end_logits[end_index],
                                # confidence_score=0
                                confidence_score=(sigmoid(start_logits[start_index])+sigmoid(end_logits[end_index]))/2
                            )
                        )
            ...
            # # Normalize start_logit and end_logit
            # # max_start_logit = max(prelim_predictions, key=lambda x: x.start_logit).start_logit
            # # max_end_logit = max(prelim_predictions, key=lambda x: x.end_logit).end_logit
            # # min_start_logit = min(prelim_predictions, key=lambda x: x.start_logit).start_logit
            # # min_end_logit = min(prelim_predictions, key=lambda x: x.end_logit).end_logit

            # # for i in range(len(prelim_predictions)):
            # #     prelim_predictions[i] = prelim_predictions[i]._replace(
            # #         start_logit=(prelim_predictions[i].start_logit - min_start_logit)/(max_start_logit - min_start_logit),
            # #         end_logit=(prelim_predictions[i].end_logit - min_end_logit)/(max_end_logit - min_end_logit)
            # #     )

            # # Create a copy of the predictions for calculating the confidence score
            # # predictions_copy = prelim_predictions.copy()
            # # for i in range(len(predictions_copy)):
            # #     predictions_copy[i] = predictions_copy[i]._replace(
            # #         start_logit=(predictions_copy[i].start_logit - min_start_logit)/(max_start_logit - min_start_logit),
            # #         end_logit=(predictions_copy[i].end_logit - min_end_logit)/(max_end_logit - min_end_logit)
            # #     )

            # prelim_predictions.sort(key=lambda x: x.feature_index)
            # grouped_predictions = {key: list(group) for key, group in groupby(prelim_predictions, key=lambda x: x.feature_index)}

            # # Calculate softmax of start_logit and end_logit independently for each group
            # for feature_index, group in grouped_predictions.items():
            #     start_logits = np.array([item.start_logit for item in group])
            #     end_logits = np.array([item.end_logit for item in group])
            #     # start_logits = (start_logits - np.min(start_logits)) / (np.max(start_logits) - np.min(start_logits))
            #     # end_logits = (end_logits - np.min(end_logits)) / (np.max(end_logits) - np.min(end_logits))
            #     # start_logits = sigmoid(start_logits)
            #     # end_logits = sigmoid(end_logits)
            #     confidence_scores = (softmax(start_logits) + softmax(end_logits))/2
            #     # confidence_scores = (start_logits + end_logits)/2
            #     # print("CONFIDENCE SCORE", confidence_scores)
            #     # softmax_end_logits = softmax(end_logits)
            #     # print(start_logits)
            #     # print(softmax_start_logits)
            #     # print(group)
            #     # break

            #     # for i, item in enumerate(group):
            #     #     print('ITEM', item)
            #     #     prelim_predictions[prelim_predictions.index(item)] = item._replace(confidence_score=(softmax_start_logits[i] + softmax_end_logits[i])/2)

            #     # Replace confidence_score with the sum of softmax of start and end logits in the original predictions
            #     for i, item in enumerate(group):
            #         prelim_predictions[prelim_predictions.index(item)] = item._replace(
            #             # confidence_score = ((confidence_scores[i] - np.min(confidence_scores)) / (np.max(confidence_scores) - np.min(confidence_scores)))
            #             confidence_score = confidence_scores[i]
            #             # confidence_score = (
            #             #     softmax((start_logits[i] - np.min(start_logits)) / (np.max(start_logits) - np.min(start_logits))) * 
            #             #     softmax((end_logits[i] - np.min(end_logits)) / (np.max(end_logits) - np.min(end_logits)))
            #             # )
            #         )
            # # print("NORMALIZED SCORE", prelim_predictions)

            #     # min_confidence_score = min(prelim_predictions, key=lambda x: x.confidence_score).confidence_score
            #     # min_confidence_score = max(prelim_predictions, key=lambda x: x.confidence_score).confidence_score
            #     # for i, item in enumerate(group):
            #     #     matching_item_index = next(
            #     #         (index for index, pred in enumerate(prelim_predictions)
            #     #         if pred.feature_index==item.feature_index and pred.start_index==item.start_index and pred.end_index==item.end_index),
            #     #         None
            #     #     )
            #     #     if matching_item_index is not None:
            #     #         prelim_predictions[matching_item_index] = prelim_predictions[matching_item_index]._replace(
            #     #             confidence_score=(softmax_start_logits[i] + softmax_end_logits[i])/2
            #             # )
            ###########################################

            prelim_predictions = sorted(prelim_predictions, key=lambda x: x.confidence_score, reverse=True)
            ...
            for rcd in prelim_predictions:
                ...
                if question_id in predictions[example_id]:
                    ...
                    predictions[example_id][question_id]["confidence_score"].append(rcd.confidence_score)
                else:
                    predictions[example_id][question_id] = {
                        "question": question,
                        "answer": [final_text],
                        "confidence_score": [rcd.confidence_score]
                    }
    ...
    def predict(self, docs):
        ...
        # print(preprocess_result.keys())
        # print(preprocess_result['tokens'])
        # print(preprocess_result['bbox'])
        # print(len(preprocess_result['tokens'][0]))
        # print(len(preprocess_result['bbox'][0][0]))
        # print('TEXT', preprocess_result['text'])
        # print('BBOX', preprocess_result['bbox'])
        # print(len(preprocess_result['text'][0]))
        # print(len(preprocess_result['bbox'][0]))
